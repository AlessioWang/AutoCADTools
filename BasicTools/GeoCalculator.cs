        ...
        public static int InsidePolygon(int vcount, Point2D[] Polygon, Point2D q)
        {
            ...
            for (i = 0; i < vcount; i++)
            {
                ...
                r1 = Multiply(Polygon[i], Polygon[(i + 1) % n], l1.s) *
                     Multiply(Polygon[(i + 1) % n], Polygon[(i + 2) % n], l1.s);
                r2 = Multiply(Polygon[i], Polygon[(i + 2) % n], l1.s) *
                     Multiply(Polygon[(i + 2) % n], Polygon[(i + 3) % n], l1.s);
            ...
            for (i = 0; i < top - 1; i++) /* 冒泡排序，x坐标小的排在前面；x坐标相同者，y坐标小的排在前面 */
            {
                for (j = i + 1; j < top; j++)
                {
                    if (PointSet[i].X > PointSet[j].X ||
                        Math.Abs(PointSet[i].X - PointSet[j].X) < EP && PointSet[i].Y > PointSet[j].Y)
                    {
                        tmp = PointSet[i];
        ...
        **********************************************************************************************/
            ...
            ref double wtr, ref double xtl, ref double ytl, ref double wtl)
        {
            ...
            if (x2 > x1)
            {
                ...
                AddPosPart((x1 + x2 + x2) / 3, (y1 + y1 + y2) / 3, (x2 - x1) * (y2 - y1) / 2, ref xtr, ref ytr,
                    ref wtr);
                // triangle 全局变量变化处 
            }
            else
            {
                ...
                // rectangle 全局变量变化处 
                AddNegPart((x1 + x2 + x2) / 3, (y1 + y1 + y2) / 3, (x2 - x1) * (y2 - y1) / 2, ref xtl, ref ytl,
                    ref wtl);
                // triangle  全局变量变化处 
            }
        }
